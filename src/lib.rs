use std::{collections::BTreeMap, intrinsics::copy_nonoverlapping, convert::TryInto};

use libipld::{codec::Codec, ipld, error::Error, Multihash, cid::CidGeneric};
...
/*
extern {
    fn load_block(cid_bytes: *const u8, cid_length: u8, block_len : &mut u32) -> *const u8;
}*/

unsafe fn load_block(cid_bytes: *const u8, cid_length: u8, block_len : &mut u32) -> *const u8 {
    let res = load_block_err(cid_bytes, cid_length, block_len);
    match res {
        Err(_) => panic!("error"),
        Ok(r) => r
    }
...
#[no_mangle]
pub unsafe fn read_adl(adlptr: *mut u8, bufptr : *mut u8, buflen : u32) -> u32  {
    let mut f : FileReader = std::ptr::read(adlptr as *const _) ;
    ...
    while at < f.length && bufrem > 0 {
        ...
        match f.cached_blocks.get(&blk_cid) {
            Some(blk) => {
                block_data = blk
            },
            None => {
                ...
                let blk_ptr = load_block(cidptr, blk_cid_bytes.len() as u8, &mut block_len);
                
                block_data = ::std::slice::from_raw_parts(
                    blk_ptr,
                    block_len as usize,
                );
            },
		...
		if at < f.offset {
            let delta = (f.offset - at) as isize;
            let block_start = block_data.as_ptr().offset(delta);
            ...
            let numcpy = num_to_copy.try_into().unwrap();

            let buf_offset = buflen - bufrem;
            copy_nonoverlapping(block_start, bufptr.offset(buf_offset as isize), numcpy);
            ...
            let buf_offset = buflen - bufrem;
            copy_nonoverlapping(block_data.as_ptr(), bufptr.offset(buf_offset as isize), numcpy);
            bufrem-=numcpy as u32;
            at += numcpy as u64;
        }
...
fn ipld_try_map(i : ipld::Ipld) -> Result<BTreeMap<String, ipld::Ipld>, Error> {
    match i {
        ipld::Ipld::Map(val) => return Ok(val),
        _ => return Err(libipld::error::Error::msg("not a map"))
    }
}

fn ipld_try_int(i : ipld::Ipld) -> Result<i128, Error> {
    match i {
        ipld::Ipld::Integer(val) => return Ok(val),
        _ => return Err(libipld::error::Error::msg("not an integer"))
    }
}

fn ipld_try_bytes(i : ipld::Ipld) -> Result<Vec<u8>, Error> {
    match i {
        ipld::Ipld::Bytes(val) => return Ok(val),
        _ => return Err(libipld::error::Error::msg("not bytes"))
    }
}

fn load_adl_internal<'a>(input: &[u8]) -> Result<Vec<u8>, Error> {
            // Assume node is DAG-CBOR
            let node : libipld::ipld::Ipld = libipld::codec_impl::IpldCodec::DagCbor.decode(&input)?;
            let node_map = ipld_try_map(node)?;

            let pieces_node = node_map.get("pieces")
                .ok_or(libipld::error::Error::msg("pieces not in node"))?;
            let pieces_bytes = ipld_try_bytes(pieces_node.to_owned())?;
